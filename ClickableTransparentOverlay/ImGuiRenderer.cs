namespace ClickableTransparentOverlay
{
  ...
  unsafe internal sealed class ImGuiRenderer : IDisposable
  {
      ...
      for (int n = 0; n < data.CmdListsCount; n++)
      {
        ...
        for (int i = 0; i < cmdList.CmdBuffer.Size; i++)
        {
          ...
          else
          {
            ...
            if (textureResources.TryGetValue(cmd.GetTexID().Handle, out var texture))
            {
    ...
    public ImTextureID CreateImageTexture(Image<Rgba32> image, Format format)
    {
    ...
    public bool RemoveImageTexture(ImTextureID handle)
    {
    ...
    {
      var imguiID = new ImTextureID(texture.NativePointer);
