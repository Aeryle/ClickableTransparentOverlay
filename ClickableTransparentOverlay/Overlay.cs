namespace ClickableTransparentOverlay
{
  ...
  public abstract class Overlay : IDisposable
  {
    ...
    private Dictionary<string, (ImTextureID Handle, uint Width, uint Height)> loadedTexturesPtrs;
    ...
    /// <returns>>true if the font replacement is valid otherwise false.</returns>
    public unsafe bool ReplaceFont(string pathName, int size, uint[] glyphRange)
    {
      if (!File.Exists(pathName))
      {
        return false;
      ...
      {
        var io = ImGui.GetIO();
        fixed (uint* p = &glyphRange[0])
        {
          // io.Fonts.AddFontFromFileTTF(pathName, size, config, new IntPtr(p));
          io.Fonts.AddFontFromFileTTF(pathName, size, config, p);
    ...
    public void AddOrGetImagePointer(string filePath, bool srgb, out ImTextureID handle, out uint width, out uint height)
    {
    ...
    /// <param name="srgb"> a value indicating whether pixel format is srgb or not.</param>
    /// <param name="handle">output pointer to the image in the graphic device.</param>
    public void AddOrGetImagePointer(string name, Image<Rgba32> image, bool srgb, out ImTextureID handle)
    {
